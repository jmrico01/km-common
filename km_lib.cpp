...
{
	DEBUG_ASSERTF(size < S, "size %lu, S %lu\n", size, S);
	return &data[size++];
}
...
void FixedArray<T, S>::Append(const Array<T>& array)
{
	...
	DEBUG_ASSERTF(newSize <= S, "size %lu, S %lu, array.size %lu\n", size, S, array.size);
